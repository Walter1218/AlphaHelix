...
| 维度 | Qlib | AlphaHelix | 评价 |
|---|---|---|---|
| 模型种类 | 20+ 种 | 双模型融合 | ✅ 我们有融合策略 |
| 模型目标 | 回归（MSE） | 回归（对数收益） | ✅ 相当 |
| 超参数调优 | 配置文件驱动 | 手动调优 | ⚠️ 需要自动化 |
| 模型集成 | DoubleEnsemble | 短期+长期融合 | ✅ 我们有创新 |
...
| 交易成本模型 | ✅ 完成 | 回测结果更真实 |
| 换仓限制 | ✅ 完成 | 降低交易成本 |
| 短窗口训练 | ✅ 完成 | IC 从 -0.06 → 0.03 |
| 双模型融合 | ✅ 完成 | 胜率 +4.4%, 收益 +14.8% |
| 飞书推送 | ✅ 完成 | 每天 19:00 自动推送 |

### 第二阶段（进行中）

| 任务 | 状态 | 预期效果 |
|---|---|---|
| 基本面数据整合 | ⚠️ 有泄漏问题 | 需要修复时间对齐 |
| 更多模型集成 | 🔄 实验中 | 可能提升 IC |
| 动态召回策略 | 🔄 实验中 | 可能提升胜率 |
...
| 改进项 | 实际效果 |
|---|---|
| 短窗口训练 | IC 从 -0.06 → 0.03 |
| 双模型融合 | 胜率 +4.4%, 收益 +14.8% |
| 交易成本模型 | 回测更真实 |
| 换仓限制 | 降低交易成本 |
| 飞书推送 | 每天自动推送 |
...
**IC 0.03 超过 Qlib 大多数模型**（Qlib LightGBM IC=0.045）。
