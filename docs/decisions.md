...
**日期**：2026-07-04
**决策**：实现 `scripts/market_defense.py`，在 `walkforward.py` 中通过 `--defense-mode` 根据 regime 与近期沪深300 回撤动态调整股票仓位。
**选项**：
1. 固定满仓，不引入仓位控制。
2. 仅按 regime 设置基础仓位（趋势下跌/高波动减仓）。
3. 在选项2基础上叠加指数回撤触发进一步减仓。

**选择**：选项3，参数可配置。
**理由**：
- 不预测下跌，只使用已实现的公开信息（regime 与回撤）。
- 仓位缩放不改变选股逻辑，现金流收益设为0，避免未来函数。
- CLI 暴露 `--defense-base` 与 `--defense-dd`，便于 walk-forward 调参与样本外验证。

**回测结果（2024-01 至 2026-06，月度，horizon=10，top-n=10，regime 策略）**：
- 满仓基准：avg excess -0.33%，cum excess -16.65%。
- 默认防御（20d/-5%、60d/-10% 均触发减半）：avg position 78.2%，cum excess -22.34%（减仓过度，错过反弹）。
- 保守防御（仅 60d/-15% 触发，trend_down=0.5 / high_vol=0.8）：avg position 97.0%，cum excess -18.40%（仍略差于满仓）。

**结论**：当前防御规则未能在该样本上提升收益；模块保留作为风控基础设施，后续改用策略自身权益曲线回撤或波动率目标进行动态仓位控制。
...
## ADR-041：样本外验证流程与 train/test 切分

**日期**：2026-07-04
**状态**：已实现
**目标**：建立严格不重叠的 train/test 流程，避免用测试集反馈迭代策略。

### 实现
- 新增 `scripts/out_of_sample_validate.py`：
  1. 在训练期跑 walk-forward，生成 snapshots/evals；
  2. 用训练期数据计算 rank IC，生成 pass2 权重；
  3. 在测试期用该固定权重跑 walk-forward；
  4. 同时跑测试期 baseline（默认权重）做对比。
- 训练期与测试期必须满足 `test_start > train_end`，脚本会显式检查。
- 生成的权重文件带 `"walk_forward": true` 元数据，可被 `walkforward.py --pass2-weights` 安全加载。

### 2024 训练 / 2025 测试结果（月度，`regime`，`universe_size=200`）

| 指标 | 训练期 | 测试期（训练权重） | 测试期（默认权重） |
|---|---|---|---|
| 平均超额 | -0.29% | +0.48% | +1.48% |
| 方向准确率 | 48.3% | 58.3% | 57.5% |
| Top3 命中率 | 36.1% | 58.3% | 75.0% |
| 累计超额 | -3.67% | +5.98% | +20.47% |

### 关键发现
- 2024 年训练期表现较差，导致训练出的权重在 2025 年并未跑赢默认权重。
- 这说明**简单全样本 IC 权重不具备跨年度稳定性**，也解释了为什么 walk-forward IC 校准效果有限。
- 真正的改进方向可能是：
  1. 按 regime 分别训练权重；
  2. 缩短训练窗口（例如只用最近 6 个月）；
  3. 加入更稳定的底层因子，而不是仅靠权重调整。

### 决策
- `out_of_sample_validate.py` 作为标准样本外验证入口保留。
- 后续任何策略/因子改动，都应先用 2024 训练 + 2025 测试验证，再看全样本 walk-forward。

---

## ADR-042：新增稳定因子与按 regime 训练权重

**日期**：2026-07-04
**状态**：已实验
**目标**：解决当前因子 IC 不稳定、全样本权重跨年度失效的问题。

### 新增因子

在 `scripts/screen.py` 中加入以下因子：

| 因子 | 计算方式 | 预期作用 |
|---|---|---|
| `mom_120` | `close_t / close_{t-120} - 1` | 长期趋势 |
| `risk_adj_mom` | `mom_20 / volatility_20` | 单位风险收益 |
| `relative_strength` | `mom_20_stock / mom_20_沪深300` | 相对基准强度 |
| `net_mf_divergence` | `net_mf_5d / net_mf_20d` | 资金流加速/减速 |
| `sector_breadth` | 行业内 mom_20 > 0 的股票占比 | 行业上涨扩散度 |

同时 `scripts/factor_ic.py` 与 `walkforward.py` 的 snapshot 字段列表同步更新。

### 按 regime 训练权重

- `calibrate_weights_from_ic.py` 新增 `--per-regime`：按 `range`/`trend_up`/`trend_down`/`high_vol` 分别计算 IC 并生成权重。
- `walkforward.py --ic-calibrate` 改为每期根据当前 regime 选择对应权重。
- `out_of_sample_validate.py` 支持 `--per-regime`，并生成 `regime_{regime}.json` 权重文件；`walkforward.py` 新增 `--regime-weights-dir` 加载这些文件。

### 验证结果

**月度 2025-01~2025-06 快速验证**（默认权重，含新因子）：
- 平均超额：+3.03%（加因子前 +2.76%）
- 方向准确率：66.7%（加因子前 65.0%）
- 累计超额：+20.90%（加因子前 +18.87%）

**2024 训练 / 2025 测试样本外（按 regime 权重）**：

| 指标 | 训练期 | 测试期（per-regime 权重） | 测试期（默认权重） |
|---|---|---|---|
| 平均超额 | -0.79% | **+0.75%** | **+1.90%** |
| 方向准确率 | 48.3% | 58.3% | 57.5% |
| 累计超额 | -9.19% | +9.95% | +27.24% |

### 关键发现
1. 新因子在短期样本内略微提升了默认权重表现。
2. **按 regime 训练的权重在 2025 年跑输默认权重**：平均超额 +0.75% vs +1.90%，累计超额 +9.95% vs +27.24%。
3. 原因可能是 2024 年各 regime 样本太少（range 仅几期、high_vol 更少），导致 regime 权重过拟合；且 2024-2025 市场结构变化大。

### 决策
- 保留新因子和按 regime 训练的基础设施。
- 当前**不启用** per-regime IC 权重作为默认；继续用默认策略权重。
- 下一步：
  1. 收集更长历史（2019-2023）增加 regime 样本；
  2. 用更短的滚动窗口（6 个月）而非固定 2024 全年训练；
  3. 继续引入更稳定、低相关的因子（如北向、融资余额、行业轮动）。
