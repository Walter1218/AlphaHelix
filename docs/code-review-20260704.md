...
3. ~~`concurrent_map` 异常处理~~：已改为任一任务异常立即抛出，避免静默混入异常对象。
4. ~~`evaluate.py` 截面缓存~~：已重构为按 `trade_date` 读取截面缓存，减少 per-stock API 调用。
5. ~~更严格的 out-of-sample 流程~~：已新增 `scripts/out_of_sample_validate.py`，强制 train/test 不重叠。

## 5. 实验后发现的新问题

- 2024 年训练出的 IC 权重在 2025 年未跑赢默认权重（平均超额 +0.48% vs +1.48%），说明当前因子 IC 不稳定、跨年度泛化能力差。
- 下一步应优先引入更稳定的底层因子或按 regime 训练，而不是继续在全样本上调整权重。
