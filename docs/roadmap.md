...
- 已验证：
  ...
  - `walkforward.py` 已完成 **30 个月回测**（2024-01 至 2026-06）。
   - `regime` 策略更新映射后全样本表现（加入新稳定因子后）：平均超额 -0.33%、方向准确率 48.7%、累计超额 -16.65%。
...
3. **[已完成] Tushare 并发预取与数据隔离** ✅
   - `scripts/prefetch_data.py` 已落地，`ALPHAHELIX_DATA_WINDOW_START/END` 已生效
   - `screen.py` 改为按 `trade_date` 截面加载日线/资金流，pass1/pass2 已并行化
4. **[已完成/待优化] 在线学习 `--online-update`** ✅
   - `walkforward.py` 已支持 regime 条件滚动权重
   - 首版验证：同参数下在线学习方向准确率 51.5%（+2.1pp），但累计超额 -6.13% 劣于静态 -4.44%；周度在线也劣于静态
   - 当前默认不启用，待优化更新规则后再开启
5. **[已完成/可扩展] 周度再平衡**
   - `walkforward.py --freq weekly` 已支持
   - 静态周度平均超额 +0.60% 明显优于月度 +0.02%
6. **[待扩展工具] pass1 权重 / regime 条件优化**
   - `multi_objective_optimizer.py` 当前只优化 pass2，需扩展 pass1 或分 regime
   - 改变候选池比改变 top-n 排序更有可能提升准确率
7. **[未实现] 接入披露日期预告 `disclosure_date`**
   - 基于财报披露时间做事件前置布局
8. **[未实现] 分行业命中率反馈**
...
12. **[已完成/实验性] 按 regime 训练权重**
   - `calibrate_weights_from_ic.py --per-regime` 与 `walkforward.py --regime-weights-dir` 已落地
   - 2024 训练 / 2025 测试：per-regime 权重平均超额 +0.75%，默认权重 +1.90%，per-regime 未跑赢
   - 下一步：增加历史 regime 样本、缩短滚动训练窗口
13. **[已完成/可继续扩展] 新增稳定因子**
   - 已加入 `mom_120`、`risk_adj_mom`、`relative_strength`、`net_mf_divergence`、`sector_breadth`
   - 2025 H1 默认权重表现小幅提升（+2.76% -> +3.03%）
   - 下一步：继续加入北向资金、融资余额、行业轮动等外部数据源
14. **[已完成] 样本外验证流程**
   - `scripts/out_of_sample_validate.py` 已落地，强制 train/test 不重叠
15. **[已完成/实验性] 熊市/急跌防御机制**
    - `scripts/market_defense.py` + `walkforward.py --defense-mode` 已落地，支持 `--defense-base` / `--defense-dd` 参数
    - 默认防御规则在该 30 个月样本上未能提升收益（cum excess -22.34% vs 满仓 -16.65%），规则有待改用策略自身回撤/波动率目标
16. **[未实现/后置] 自动化调度 / 交易成本 / DPO / 新数据源**（accuracy 稳定后再做）
...
- [x] 验证周度再平衡：静态周度平均超额 +0.60% 优于月度 +0.02%；但在线学习周度 +0.01% 劣于静态
