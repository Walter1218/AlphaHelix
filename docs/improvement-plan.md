...
### 10.3 L5 评估：扩展回测样本（已完成，结果需重视）

已运行 `walkforward.py --start 20240101 --end 20260615`，覆盖 **30 个月**。
...
**关键发现**：
1. 2025 年表现强劲，但 2024 年与 2026 H1 明显拖后腿。
2. 早期 8 个月样本跳过了 2026 年 1-2 月的大跌，存在样本选择偏差。
3. 策略在熊市/急跌环境（2024 多个月、2026-01/02）适应性不足，急需 regime 条件权重和风控增强。
...
| 层级 | 指标 | 当前 | 3 周后目标 |
|---|---|---|---|
| L5 评估 | 方向准确率 | 49.4%（30 个月平均，regime 新映射） | ≥ 55% |
| L5 评估 | 月度超额收益 | +0.02%（30 个月平均，regime 新映射） | > 1.0% |
| L5 评估 | 组合最大回撤 | 单期 -9.16% | < -7% |
| L5 评估 | 回测样本数 | 8 个月 | ≥ 12 个月 |
| L5 评估 | 交易成本 | 未扣除 | 已扣除印花税+佣金+滑点 |
| L3 风控 | 行业集中度 | 数量控制已落地 | 市值权重 ≤ 40% |
| L2 策略 | event_driven 累计超额 | 2025 +11.60%，2026 Q2 +8.99% | 纳入 regime 后整体更优 |
| L2 策略 | quality_growth 累计超额 | 接近 0% | > 2% |
| L6 Harness | 在线学习 | 手动运行 | `--auto` 增量更新 |
| L6 Harness | 分行业命中率反馈 | 无 | 覆盖主要行业 |
| L4 Agent | Trace 覆盖 | 脚本层 + agent reasoning 已落地 | DPO 数据集导出 |
