...
| C38 | 权重/策略优化必须 walk-forward | 禁止用全样本 IC 或全样本网格搜索生成固定权重后回测同一区间；只能用该期之前的数据生成当期权重 |

违反上述纪律的选股结果不得输出。

### 2.5 回测与权重优化的红线

> 本项纪律是对 C01「无未来函数」在**策略优化/权重校准**场景下的细化，必须写进所有相关脚本与文档。

**禁止行为**：

1. **全样本 IC 优化后回测同一区间**：
   - 错误做法：用 2024-01 到 2026-06 全部数据计算因子 IC，生成一套固定权重，再回测 2024-01 到 2026-06。
   - 原因：对 2024-01 来说，权重已经「看到」了 2024-02 之后的数据，属于时间穿越。

2. **全样本网格搜索后回测同一区间**：
   - 错误做法：在整个回测区间上搜索最优 `learning_rate`/`lookback`/`min_score`，再用该参数回测同一区间。

3. **把样本内优化结果当作有效绩效对外报告**：
   - 任何基于全样本调参后的回测结果，只能作为「诊断/上限参考」，不能写入 `memory/eval/` 作为正式结果，也不能用于生产权重。

**允许做法**：

1. **Walk-forward 校准**：
   - 每期用该期之前的数据生成权重/参数，再用于当期选股。
   - 例如 `walkforward.py --ic-calibrate`、滚动窗口 IC 更新。

2. **样本外验证**：
   - 用 2024 年数据训练/调参，用 2025-2026 年数据测试，训练集和测试集不重叠。

3. **诊断性分析**：
   - 可以计算全样本因子 IC 来发现权重方向问题，但必须在文档和输出中明确标注「样本内诊断，非有效回测」。
