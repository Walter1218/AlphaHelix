"""
AlphaHelix 分 regime GBDT 滚动回测

流程：
1. 读取 dataset 并用 walk_forward_predict_by_regime 生成预测；
2. 每个再平衡日按对应 regime 的 GBDT 得分选 top-k；
3. 走完整 portfolio_backtest（含交易成本、可选宏观仓位缩放）。

与 walkforward_gbdt.py 的区别：
- 每个 fold 按 strong/neutral/weak 训练专用模型；
- 预测日根据当日宏观状态选择模型输出。
"""
import sys
import os
...
from model_trainer import load_dataset, get_feature_cols, walk_forward_predict_by_regime
...
def generate_predictions(dataset_path: str, macro_dataset: str, horizon: int,
                         model_type: str, train_window_months: int,
                         target: str, pos_thr: float, neg_thr: float) -> pd.DataFrame:
    df = load_dataset(horizon, dataset_path)
    feature_cols = get_feature_cols(df)
    pred_df = walk_forward_predict_by_regime(
        df, feature_cols,
    ...
    output_name = f"predictions_h{horizon}_regime_{target}_{model_type}.parquet"
    output_path = PRED_DIR / output_name
    pred_df.to_parquet(output_path, index=False)
    print(f"[walkforward_regime_gbdt] Saved predictions to {output_path}")
    return pred_df


def run_walkforward_regime_gbdt(dataset_path: str,
                                macro_dataset: str,
                                horizon: int = 10,
    ...
    pred_df = generate_predictions(dataset_path, macro_dataset, horizon,
                                   model_type, train_window_months, target,
                                   pos_thr, neg_thr)
    ...
    if use_wf_threshold:
        print(f"[walkforward_regime_gbdt] Applying walk-forward threshold calibration "
              f"(train_periods={wf_train_periods}, metric={wf_metric})")
        pred_tmp = PRED_DIR / f"_tmp_regime_pred_for_wfthresh_{datetime.now().strftime('%H%M%S')}.parquet"
        pred_df.to_parquet(pred_tmp, index=False)
        masked_path = PRED_DIR / f"predictions_h{horizon}_regime_{target}_{model_type}_wfthresh.parquet"
        calibrate_and_mask(
    ...
    tmp_path = PRED_DIR / f"_tmp_walkforward_regime_gbdt_{datetime.now().strftime('%H%M%S')}.parquet"
    ...
    summary["pred_path"] = str(dataset_path)
    summary["macro_dataset"] = str(macro_dataset)
    ...
    summary["pos_thr"] = pos_thr
    summary["neg_thr"] = neg_thr

    summary_path = OUTPUT_DIR / f"gbdt_regime_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print(f"[walkforward_regime_gbdt] Summary saved to {summary_path}")
...
def main():
    ...
    parser.add_argument("--macro-dataset", required=True, help="含宏观特征的 parquet")
    ...
    print("\n=== Walk-forward Regime GBDT Backtest Summary ===")
